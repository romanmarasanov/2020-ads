...
public class HashTableOnChains<K, V> implements HashTable<K, V> {
    ...
    public void put(@NotNull K k, @NotNull V v) {
        ...
        if (res == null) {
            ...
            size++;
            if (((double)size / (double)table.length) > LOAD_FACTOR) resize();
            return;
        }
        while (!res.key.equals(k) && res.next != null){
            res = res.next;
        }
    ...
    public @Nullable V remove(@NotNull K k) {
        ...
        while (res.next != null && !res.next.key.equals(k)) res = res.next;
        if (res.next == null){
            V val = res.value;
            res = null;
            return val;
